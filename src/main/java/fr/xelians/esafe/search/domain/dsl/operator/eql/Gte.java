 ...
 */
@ToString
public class Gte extends GteOperator<Query> {

  // { "$gte": { "client.age": 33 } , "$type": "Company"  }
  public Gte(DslParser<Query> parser, SearchContext searchContext, JsonNode parameter) {
  ...
  public Query create() {
    ...
    Query query = RangeQuery.of(r -> r.field(field.getFullName()).gte(data))._toQuery();
